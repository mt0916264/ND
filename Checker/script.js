...
checkBtn.addEventListener("click", function () {
  ...
  for (let i = 0; i < numberArray.length; i++) {
    ...
    if (!isValidCreditCard(cardNumber)) {
      muradList.push(`<span style='color:grey; font-weight:bold;'>Invalid (Luhn Check)</span> | ${line} /NdCheckerV1`);
    ...
    if (randomNumber < 0.2) {
      aliList.push(`<span style='color:green; font-weight:bold;'>Live</span> | ${line} -> [Charge <span style='color:green; font-weight:bold;'>R$1,39</span>] [GATE:01]. /NdCheckerV1`);
    } else if (randomNumber < 0.9) {
      muhammadList.push(`<span style='color:red; font-weight:bold;'>Dead</span> | ${line} -> [Charge <span style='color:red; font-weight:bold;'>R$0,00</span>] [GATE:01]. /NdCheckerV1`);
    } else {
      muradList.push(`<span style='color:orange; font-weight:bold;'>Unknown</span> | ${line} -> [Charge <span style='color:orange; font-weight:bold;'>N/A</span>] [GATE:01]. /NdCheckerV1`);
    }
  } 
  ...
  updateNumbers = setInterval(() => {
    ...
    if (i >= aliList.length && i >= muhammadList.length && i >= muradList.length) {
      ...
      Swal.fire({
        title: "Checker finalizado!",
        text: "Todos os cartões foram verificados com sucesso.",
        icon: "success",
...
stopCheckBtn.addEventListener("click", function () {
  clearInterval(updateNumbers);
  Swal.fire({
    title: "Checker cancelado!",
    text: "O processo de verificação foi interrompido.",
    icon: "error",
